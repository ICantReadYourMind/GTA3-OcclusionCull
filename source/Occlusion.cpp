...
int32_t COcclusion::NumOccludersOnMap;
int32_t COcclusion::FarAwayList;
int16_t COcclusion::NearbyList;
int16_t COcclusion::ListWalkThroughFA;
int16_t COcclusion::PreviousListWalkThroughFA;
int16_t COcclusion::NumActiveOccluders;
//COccluder COcclusion::aOccluders[NUMOCCLUSIONVOLUMES];
std::vector<COccluder> COcclusion::aOccluders;
...
COcclusion::AddOne(float x, float y, float z, float width, float length, float height, float angle)
{
	...
	occluder.length = length;
	occluder.height = height;
	while(angle < 0.0f) angle += 360.0f;
	while(angle > 360.0f) angle -= 360.0f;
	occluder.angle = angle/360.0f * UINT16_MAX;
	occluder.listIndex = FarAwayList;
	//FarAwayList = NumOccludersOnMap++;
	FarAwayList = aOccluders.size();
	aOccluders.push_back(occluder);
}
...
bool CalcScreenCoors(CVector const &in, CVector *out, float *outw, float *outh) {
	*out = TheCamera.m_ViewMatrix * in;
	...
	float fovScale = DefaultFOV / CDraw::ms_fFOV;
...
COcclusion::ProcessBeforeRendering(void)
{
	...
	//if (CGame::currLevel != LEVEL_MAIN_MA)
	//	return;
...
void COcclusion::Render() {
	...
	float recipz = 1.0f/RwCameraGetNearClipPlane(Scene.m_pCamera);
	for (int i = 0; i < NumActiveOccluders; i++) {
...
CVector
GetBoundCentre(CEntity* ent)
{
	return ent->GetMatrix() * CModelInfo::ms_modelInfoPtrs[ent->m_nModelIndex]->m_pColModel->m_boundSphere.m_vecCenter;
}
...
bool IsEntityOccluded(CEntity* ent) {
	...
	if (COcclusion::NumActiveOccluders == 0 || !CalcScreenCoors(GetBoundCentre(ent), &coors, &width, &height))
		return false;

	float area = Max(width, height) * ent->GetBoundRadius() * 0.9f;

	for (int i = 0; i < COcclusion::NumActiveOccluders; i++) {
		if (coors.z - (ent->GetBoundRadius() * 0.85f) > COcclusion::aActiveOccluders[i].radius) {
			...
			if (COcclusion::aActiveOccluders[i].IsPointWithinOcclusionArea(coors.x, coors.y, 0.0f)) {
				CVector min = ent->m_matrix * CModelInfo::ms_modelInfoPtrs[ent->m_nModelIndex]->m_pColModel->m_boundBox.m_vecMin;
				CVector max = ent->m_matrix * CModelInfo::ms_modelInfoPtrs[ent->m_nModelIndex]->m_pColModel->m_boundBox.m_vecMax;
